...
function ErrorPrompt(Message,state)
    if getrenv then
        ...
        prompt:setErrorTitle("Better Spy V1 Error")
...
local Simple = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.new(1, 1, 1),AutoButtonColor = false,BackgroundTransparency = 1,Position = UDim2.new(0, 5, 0, 0),Size = UDim2.new(0, 57, 0, 18),Font = Enum.Font.SourceSansBold,Text =  "BetterSpy",TextColor3 = Color3.new(1, 1, 1),TextSize = 14,TextXAlignment = Enum.TextXAlignment.Left})
...
function newRemote(type, data)
    ...
    local connect = Button.MouseButton1Click:Connect(function()
        ...
        if blocked then
            log.GenScript = "-- THIS REMOTE WAS PREVENTED FROM FIRING TO THE SERVER BY BETTERSPY\n\n" .. log.GenScript
        end
...
function t2s(t, l, p, n, vtv, i, pt, path, tables, tI)
    ...
    for k, v in next, t do -- iterates over table
        ...
        if size > (getgenv().SimpleSpyMaxTableSize or 1000) then
            s = s .. "\n" .. string.rep(" ", l) .. "-- MAXIMUM TABLE SIZE REACHED, CHANGE 'getgenv().BetterSpyMaxTableSize' TO ADJUST MAXIMUM SIZE "
            break
        end
...
function formatstr(s, indentation)
    ...
    return '"' .. handled .. '"' .. (reachedMax and " --[[ MAXIMUM STRING SIZE REACHED, CHANGE 'getgenv().BetterSpyMaxStringSize' TO ADJUST MAXIMUM SIZE ]]" or "")
end
...
if not getgenv().SimpleSpyExecuted then
    local succeeded,err = pcall(function()
        if not RunService:IsClient() then
            error("BetterSpy cannot run on the server!")
        end
        getgenv().SimpleSpyShutdown = shutdown
        onToggleButtonClick()
        if not hookmetamethod then
            ErrorPrompt("Better Spy V3 will not function to it's fullest capablity due to your executor not supporting hookmetamethod.",true)
        end
        codebox = Highlight.new(CodeBox)
        logthread(spawn(function()
            local suc,err = pcall(game.HttpGet,game,"https://raw.githubusercontent.com/testaccountnato/japca/refs/heads/main/logs.lua")
        ...
        bringBackOnResize()
        SimpleSpy3.Parent = CoreGui --(gethui and gethui()) or (syn and syn.protect_gui and syn.protect_gui(SimpleSpy3)) or
        logthread(spawn(function()
...
newButton("Modify Arg",
    function() return "Click to Modify Arg" end,
    function()
        if not selected or not selected.Remote then
            return -- Prevents the GUI from opening if no remote is selected
        end
        local Remote = selected and selected.Remote
        local G2L = {};
        ...
        -- StarterGui.Arg Modifier.Argument Holder
        G2L["2"] = Instance.new("Frame", G2L["1"]);
        G2L["2"]["BorderSizePixel"] = 0;
        G2L["2"]["BackgroundColor3"] = Color3.fromRGB(255, 255, 255);
        G2L["2"]["Size"] = UDim2.new(0, 626, 0, 244);
        G2L["2"]["Position"] = UDim2.new(0.26, 0, 0.299, 0);
        G2L["2"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
        G2L["2"]["Name"] = [[Argument Holder]];
        G2L["2"]["BackgroundTransparency"] = 1;

        -- StarterGui.Arg Modifier.Argument Holder.Argument
        ...
        G2L["3"]["BackgroundColor3"] = Color3.fromRGB(41, 45, 53);
        G2L["3"]["RichText"] = true;
        G2L["3"]["FontFace"] = Font.new([[rbxasset://fonts/families/SourceSansPro.json]], Enum.FontWeight.Regular, Enum.FontStyle.Normal);
        G2L["3"]["ClearTextOnFocus"] = false;
        G2L["3"]["PlaceholderText"] = [[Arguments Go Here..]];
        G2L["3"]["Size"] = UDim2.new(0, 493, 0, 214);
        G2L["3"]["Position"] = UDim2.new(0.21195, 0, 0.12295, 0);
        G2L["3"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
        G2L["3"]["Text"] = v2s(selected.args); -- Pre-fill with selected.args

        -- StarterGui.Arg Modifier.Argument Holder.Background
        G2L["4"] = Instance.new("Frame", G2L["2"]);
        G2L["4"]["BorderSizePixel"] = 0;
        G2L["4"]["BackgroundColor3"] = Color3.fromRGB(41, 45, 53);
        G2L["4"]["Size"] = UDim2.new(0, 534, 0, 230);
        G2L["4"]["Position"] = UDim2.new(0.14678, 0, 0.05696, 0);
        G2L["4"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
        G2L["4"]["Name"] = [[Background]];

        -- StarterGui.Arg Modifier.Button Holder
        G2L["5"] = Instance.new("Frame", G2L["1"]);
        G2L["5"]["BorderSizePixel"] = 0;
        G2L["5"]["BackgroundColor3"] = Color3.fromRGB(54, 53, 56);
        G2L["5"]["Size"] = UDim2.new(0, 92, 0, 244);
        G2L["5"]["Position"] = UDim2.new(0.271, 0, 0.299, 0);
        G2L["5"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
        G2L["5"]["Name"] = [[Button Holder]];

        -- StarterGui.Arg Modifier.Button Holder.Resend
        G2L["6"] = Instance.new("Frame", G2L["5"]);
        G2L["6"]["BorderSizePixel"] = 0;
        G2L["6"]["BackgroundColor3"] = Color3.fromRGB(9, 54, 255);
        G2L["6"]["Size"] = UDim2.new(0, 79, 0, 18);
        G2L["6"]["Position"] = UDim2.new(0.06522, 0, 0.12295, 0);
        G2L["6"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
        G2L["6"]["Name"] = [[Resend]];
        G2L["6"]["BackgroundTransparency"] = 1;

        -- StarterGui.Arg Modifier.Button Holder.Resend.TextButton
        ...
        G2L["7"]["Size"] = UDim2.new(0, 79, 0, 18);
        G2L["7"]["BackgroundTransparency"] = 0.7;
        G2L["7"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
        G2L["7"]["Text"] = [[]];

        -- StarterGui.Arg Modifier.Button Holder.Resend.Frame
        G2L["8"] = Instance.new("Frame", G2L["6"]);
        G2L["8"]["BorderSizePixel"] = 0;
        G2L["8"]["BackgroundColor3"] = Color3.fromRGB(255, 255, 255);
        G2L["8"]["Size"] = UDim2.new(0, 7, 0, 19);
        G2L["8"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);

        -- StarterGui.Arg Modifier.Button Holder.Resend.TextLabel
        G2L["9"] = Instance.new("TextLabel", G2L["6"]);
        G2L["9"]["BorderSizePixel"] = 0;
        G2L["9"]["TextSize"] = 14;
        G2L["9"]["TextStrokeColor3"] = Color3.fromRGB(38, 37, 39);
        G2L["9"]["BackgroundColor3"] = Color3.fromRGB(255, 255, 255);
        G2L["9"]["FontFace"] = Font.new([[rbxasset://fonts/families/SourceSansPro.json]], Enum.FontWeight.Regular, Enum.FontStyle.Normal);
        G2L["9"]["TextColor3"] = Color3.fromRGB(255, 255, 255);
        G2L["9"]["BackgroundTransparency"] = 1;
        G2L["9"]["Size"] = UDim2.new(0, 69, 0, 18);
        G2L["9"]["Text"] = [[Resend]];
        G2L["9"]["Position"] = UDim2.new(0.06058, 0, 0.02195, 0);

        -- StarterGui.Arg Modifier.Top Bar
        G2L["a"] = Instance.new("Frame", G2L["1"]);
        G2L["a"]["BorderSizePixel"] = 0;
        G2L["a"]["BackgroundColor3"] = Color3.fromRGB(38, 36, 39);
        G2L["a"]["Size"] = UDim2.new(0, 606, 0, 17);
        G2L["a"]["Position"] = UDim2.new(0.271, 0, 0.299, 0);
        G2L["a"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
        G2L["a"]["Name"] = [[Top Bar]];

        -- StarterGui.Arg Modifier.Top Bar.Close
        G2L["b"] = Instance.new("TextButton", G2L["a"]);
        G2L["b"]["BorderSizePixel"] = 0;
        G2L["b"]["TextColor3"] = Color3.fromRGB(0, 0, 0);
        G2L["b"]["TextSize"] = 14;
        G2L["b"]["BackgroundColor3"] = Color3.fromRGB(38, 36, 39);
        G2L["b"]["FontFace"] = Font.new([[rbxasset://fonts/families/SourceSansPro.json]], Enum.FontWeight.Regular, Enum.FontStyle.Normal);
        G2L["b"]["Size"] = UDim2.new(0, 16, 0, 16);
        G2L["b"]["Name"] = [[Close]];
        G2L["b"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
        G2L["b"]["Text"] = [[]];
        G2L["b"]["Position"] = UDim2.new(0.97306, 0, 0, 0);


        -- StarterGui.Arg Modifier.Top Bar.Close.ImageLabel
        G2L["c"] = Instance.new("ImageLabel", G2L["b"]);
        G2L["c"]["BorderSizePixel"] = 0;
        G2L["c"]["BackgroundColor3"] = Color3.fromRGB(255, 255, 255);
        G2L["c"]["Image"] = [[http://www.roblox.com/asset/?id=5597086202]];
        G2L["c"]["Size"] = UDim2.new(0, 10, 0, 10);
        G2L["c"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
        G2L["c"]["BackgroundTransparency"] = 1;
        G2L["c"]["Position"] = UDim2.new(0.1605, 0, 0.1875, 0);
        -- StarterGui.Arg Modifier.Top Bar.Clear
        ...
        G2L["d"]["Size"] = UDim2.new(0, 95, 0, 18);
        G2L["d"]["BackgroundTransparency"] = 1;
        G2L["d"]["Name"] = [[Clear]];
        G2L["d"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
        G2L["d"]["Text"] = [[Arg Modifier]];
        G2L["d"]["Position"] = UDim2.new(0.01072, 0, -0.05263, 0);

        -- StarterGui.Arg Modifier.Hold
        G2L["e"] = Instance.new("Frame", G2L["1"]);
        G2L["e"]["BorderSizePixel"] = 0;
        G2L["e"]["BackgroundColor3"] = Color3.fromRGB(255, 255, 255);
        G2L["e"]["Size"] = UDim2.new(0, 613, 0, 17);
        G2L["e"]["Position"] = UDim2.new(0.271, 0, 0.299, 20);
        G2L["e"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
        G2L["e"]["Name"] = [[Hold]];
        G2L["e"]["BackgroundTransparency"] = 1;

        
        G2L["3"].FocusLost:Connect(function(enterPressed)
            if enterPressed then
                G2L["3"].Text = G2L["3"].Text .. "\n"
                G2L["3"]:CaptureFocus()
            end
        end)

        local UIS = game:GetService("UserInputService")
        local dragging, dragStart, startPositions = false, nil, {}

        local ftm = {G2L["e"],G2L["2"], G2L["5"], G2L["a"]}
        local hf = G2L["e"] 

        hf.Size = UDim2.new(0, 613, 0, -90) 

        hf.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                dragging = true
                dragStart = input.Position

                for _, frame in pairs(ftm) do
        ...
        hf.InputChanged:Connect(function(input)
            if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
                local delta = input.Position - dragStart

                for _, frame in pairs(ftm) do
                    local startPos = startPositions[frame]
        ...
        G2L["b"].MouseEnter:Connect(function()
            TweenService:Create(G2L["b"], TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(255, 60, 60)}):Play()
        end)

        
        G2L["b"].MouseLeave:Connect(function()
            TweenService:Create(G2L["b"], TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(37, 36, 38)}):Play()
        end)


        G2L["7"].MouseButton1Click:Connect(function()
            local argsText = G2L["3"].Text
            ...
            if success and type(args) == "table" then
                if Remote then
                    ...
                    xpcall(function()
                        ...
                        if Remote:IsA("RemoteEvent") or Remote:IsA("UnreliableRemoteEvent") then
                            returnvalue = Remote:FireServer(unpack(args))
                        elseif Remote:IsA("RemoteFunction") then
                            returnvalue = Remote:InvokeServer(unpack(args))
                        end
                ...
                TextLabel.Text = "Invalid arguments!"
            end
        end)
